...
func (u *Update) Apply() {
	...
	if u.OldState.HostNat != u.NewState.HostNat ||
		u.OldState.HostNatInterface != u.NewState.HostNatInterface {

		if u.NewState.HostNat {
			if u.OldState.HostNat {
				...
				if err != nil {
					logrus.WithFields(logrus.Fields{
						"old_host_nat_iface": u.OldState.HostNatInterface,
						"new_host_nat_iface": u.NewState.HostNatInterface,
						"error":              err,
					}).Error("iptables: Host nat iptables replace failed")
				}
			}
			_, err := utils.ExecCombinedOutputLogged(
				[]string{
			...
			if err != nil {
				logrus.WithFields(logrus.Fields{
					"host_nat_iface": u.NewState.HostNatInterface,
					"error":          err,
				}).Error("iptables: Host nat iptables add failed")
			}
		} else if u.OldState.HostNat {
			_, err := utils.ExecCombinedOutputLogged(
				[]string{
					"matching rule exist",
					"match by that name",
			...
			if err != nil {
				logrus.WithFields(logrus.Fields{
					"host_nat_iface": u.OldState.HostNatInterface,
					"error":          err,
				}).Error("iptables: Host nat iptables remove failed")
			}
		}
	}

	remNatExcludes := u.OldState.HostNatExcludes.Copy()
	remNatExcludes.Subtract(u.NewState.HostNatExcludes)
	for natExcludeInf := range remNatExcludes.Iter() {
		...
		if err != nil {
			logrus.WithFields(logrus.Fields{
				"nat_exclude": natExclude,
				"error":       err,
			}).Error("iptables: Host nat iptables exclude remove failed")
		}
	}

	addNatExcludes := u.NewState.HostNatExcludes.Copy()
	addNatExcludes.Subtract(u.OldState.HostNatExcludes)
	for natExcludeInf := range addNatExcludes.Iter() {
		...
		if err != nil {
			logrus.WithFields(logrus.Fields{
				"nat_exclude": natExclude,
				"error":       err,
			}).Error("iptables: Host nat iptables exclude add failed")
		}
	}

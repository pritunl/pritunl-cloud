...
func BuildConfig(inst *instance.Instance, virt *vm.VirtualMachine,
	unt *unit.Unit, deply *deployment.Deployment, spc *spec.Spec,
	zne *zone.Zone, vc *vpc.Vpc, subnet *vpc.Subnet, pods []*pod.Pod,
	...
	conf = &types.Config{
		...
		Deployment: types.NewDeployment(
			deply, unt != nil && deply != nil && unt.Primary == deply.Id),
		Unit:         types.NewUnit(unt),
		Vpc:          types.NewVpc(vc),

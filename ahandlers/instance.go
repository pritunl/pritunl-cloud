...
func instancePut(c *gin.Context) {
	...
	inst.CloudInterface = dta.CloudInterface
...
func instancePost(c *gin.Context) {
	...
	for i := 0; i < dta.Count; i++ {
		...
		inst := &instance.Instance{
			...
			CloudInterface:      dta.CloudInterface,
...
func instanceAdvisoryGet(c *gin.Context) {
	...
	if demo.IsDemo() {
		c.JSON(200, demo.GetResourceAdvisories(instanceId))
		return
	}

	advisories, err := aggregate.GetResourceAdvisories(db, instanceId)
	if err != nil {
		utils.AbortWithError(c, 500, err)
		return
	}

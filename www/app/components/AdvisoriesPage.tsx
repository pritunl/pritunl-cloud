...
export default class AdvisoriesPage extends React.Component<Props, State> {
	constructor(props: any, context: any) {
		super(props, context);
		this.state = {
			page: AdvisoriesStore.page,
			pageCount: AdvisoriesStore.pageCount,
			pages: AdvisoriesStore.pages,
			count: AdvisoriesStore.count,
		};
	}
	...
	render(): JSX.Element {
		...
		return <div className="layout horizontal center-justified">
			...
			<button
				...
				type="button"
				onClick={(): void => {
					AdvisoryActions.traverse(Math.max(0, this.state.page - 1));
					if (this.props.onPage) {
						this.props.onPage();
					}
				}}
			...
			<button
				...
				type="button"
				onClick={(): void => {
					AdvisoryActions.traverse(Math.min(
			...
			<button
				...
				type="button"
				onClick={(): void => {
					AdvisoryActions.traverse(this.state.pages - 1);
					if (this.props.onPage) {
						this.props.onPage();
					}
				}}
